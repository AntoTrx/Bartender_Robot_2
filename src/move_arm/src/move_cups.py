...
DEFAULT_VISION_SERVICE = "/find_cups"
...
class Bartender():
    ...
    def __init__(self):
        ...
        self.right_arm = ['right_j0', 'right_j1', 'right_j2', 'right_j3', 'right_j4', 'right_j5', 'right_j6']
        rospy.loginfo("Initializing Bartender")

        # ik service
        self.ik_service_name = DEFAULT_IK_SERVICE
        self.ik_service_proxy = rospy.ServiceProxy(self.ik_service_name, SolvePositionIK)
        self.ik_service_proxy.wait_for_service()
        rospy.loginfo("Successful connection to '" + self.ik_service_name + "'.")

        # vision service
        self.vision_service_name = DEFAULT_VISION_SERVICE
        self.vision_service_proxy = rospy.ServiceProxy(self.vision_service_name, target)
        self.vision_service_proxy.wait_for_service()
        rospy.loginfo("Successful connection to '" + self.vision_service_name + "'.")

        # check state service
        self.check_service_name = DEFAULT_CHECK_SERVICE
        # self.check_service_proxy = rospy.ServiceProxy(self.check_service_name, GetStateValidity)
        self.check_service_proxy = rospy.ServiceProxy(self.check_service_name, GetStateValidity)
        self.check_service_proxy.wait_for_service()
    ...
    def getTargetEEF(self):
        ...
        req = targetRequest()
        req.data = 0
        resp = self.vision_service_proxy.call(req)
    ...
    def convertToJointStates(self, target):
        ...
        resp = self.ik_service_proxy.call(ikreq)
    ...
    #     resp = self.ik_service_proxy(req)
    ...
    def checkCollisions(self,target):
        ...
        resp = self.check_service_proxy(req)
